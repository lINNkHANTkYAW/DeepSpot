  ...
  try {
    ...
    if (isDatabaseConfigured()) {
      try {
        ...
        return NextResponse.json({ pendingPosts: pending.map((post) => ({ ...post, createdAt: post.createdAt.toISOString() })) });
      } catch (error) {
    ...
    return NextResponse.json({ pendingPosts: pending });
  } catch (error) {
  ...
  try {
    ...
    if (isDatabaseConfigured()) {
      try {
        ...
        return NextResponse.json({ message: `Post ${id} has been ${action.toLowerCase()}d.`, post: { ...updatedPost, createdAt: updatedPost.createdAt.toISOString() } });
      } catch (error) {
